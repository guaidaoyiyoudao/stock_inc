...
interface DividendFreshnessStore {
    ...
    fun lastSuccessAt(stockCode: String): Long

    /** 该股最后一次尝试拉取分红的时间戳（含失败）；0 = 从未尝试。 */
    fun lastAttemptAt(stockCode: String): Long
...
class PrefsDividendFreshnessStore @Inject constructor(
    ...
    override fun lastSuccessAt(stockCode: String): Long = prefs.getLong(KEY_SUCCESS + stockCode, 0L)

    override fun lastAttemptAt(stockCode: String): Long = prefs.getLong(KEY_ATTEMPT + stockCode, 0L)
