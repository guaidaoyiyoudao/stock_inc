...
class GetStockInfoTool(
    ...
    override suspend fun run(context: ToolContext, args: Map<String, Any>): Any {
        ...
        return runCatching {
            ...
            val price = marketDataPlane.refreshPrice(entity)
            val saved = marketDataPlane.observeStock(stock.code).first()
            val latest = marketDataPlane.getLatestDividend(stock.code)
            // 股息率走平面 canonical 口径：DPS（自动 ensureFresh）÷ 现价。
            // dividends 表的 dividendYield 仅为除权时点历史快照（腾讯主源恒 null），不能透传。
            val yearlyCash = marketDataPlane.getDps(stock.code)
            buildMap<String, Any?> {
                put("code", stock.code)
                put("name", stock.name)
...
class GetFundamentalsTool(
    ...
    override suspend fun run(context: ToolContext, args: Map<String, Any>): Any {
        ...
        return runCatching {
            ...
            // 平面版基本面已补派息率（enrichPayoutRatio 收敛到数据平面内）
            val enriched = marketDataPlane.getFundamentals(stock.code, forceRefresh = forceRefresh)
...
class GetKlineTool(
    ...
    override suspend fun run(context: ToolContext, args: Map<String, Any>): Any {
        ...
        return runCatching {
            val stock = marketDataPlane.resolveStock(code)
                ?: return@runCatching mapOf("error" to "未找到股票：$code")
            val klines = marketDataPlane.getKlines(stock.code, KlinePeriod.valueOf(period), bars)
...
class SearchStockTool(
    ...
    override suspend fun run(context: ToolContext, args: Map<String, Any>): Any {
        ...
        return runCatching {
            val results = marketDataPlane.searchStocks(query).getOrNull().orEmpty()
            mapOf(
...
class GetBuyThresholdTool(
    ...
    override suspend fun run(context: ToolContext, args: Map<String, Any>): Any {
        ...
        return runCatching {
            ...
            val price = marketDataPlane.refreshPrice(entity)
            val saved = marketDataPlane.observeStock(stock.code).first()
            val dps = marketDataPlane.getDps(stock.code)
            val bond = marketDataPlane.get10YBondYield()
