...
class TodayViewModelTest {
    ...
    private fun makeVm() = TodayViewModel(
        ...
        diagnosisAssembler, dividendIncomeRepository,
    )
    ...
    fun marketEnvironment_filtersToFourIndicesAndSplitsMood() = runTest {
        ...
        coEvery {
            marketDataRepository.fetchIndustryList(MarketDataRepository.SortBy.INFLOW, any())
    ...
    fun emptyHoldings_marketStillPopulatedButNoDiagnosis() = runTest {
        coEvery { briefingCoordinator.read(any()) } returns null
        coEvery { marketDataRepository.fetchIndexQuotes() } returns listOf(index("000001", "上证指数", 1.0))
